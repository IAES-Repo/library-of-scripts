...
class BoxUploader:
    ...
    def upload_file(self, file_path):
        ...
        try:
            ...
            print(f"Error checking file size for {file_name}: {str(e)}")
            self.status_monitor.record_upload(0, success=False)
            return

        try:
            folder_id = self.folder_mapping[directory]
            print(f"Attempting to upload {file_name} (size: {file_size} bytes) to folder {folder_id}")

            try:
                target_folder = self.client.folder(folder_id)
                ...
                print(f"Successfully uploaded {file_name} ({uploaded_file.size} bytes) to Box folder {folder_id}")
